...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		modVersionTxt = new FlxText(bgDescription.x + 15, bgButtons.y + 15, bgDescription.width - 30, "Mod Version: 0.1.0", 16);
		modVersionTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, FlxTextAlign.LEFT);
		add(modVersionTxt);
	...
	override function update(elapsed:Float)
	{
		if(controls.BACK && hoveringOnMods)
		{
			...
			if(waitingToRestart) {
				Mods.onModChange.dispatch(modsList);
				resetGame();
			}
			else {
				Mods.onModChange.dispatch(modsList);
	...
	public static function resetGame() {
		...
		FlxG.camera.fade(FlxColor.BLACK, 0.5, false, function() {
			...
			backend.scripts.GlobalScript.destroyModScript();
			#end
			FlxG.resetGame();
		}, false);
	}
	...
	function updateModDisplayData()
	{
		...
		modVersionTxt.visible = (curMod.version != null && curMod.version.length > 0);
		if(modVersionTxt.visible) modVersionTxt.text = "Mod Version: " + curMod.version;
	...
	override function destroy() {
		#if DISCORD_ALLOWED DiscordClient.loadModRPC(); #end
		super.destroy();
	}
}
...
class ModItem extends FlxSpriteGroup
{
	...
	public var version:Null<String> = null;
		...
		if(pack != null)
		{
			...
			if(pack.color != null) {
				this.bgColor = FlxColor.fromRGB(pack.color[0] != null ? pack.color[0] : 170,
					pack.color[1] != null ? pack.color[1] : 0, pack.color[2] != null ? pack.color[2] : 255);
			}
