...
class PlayState extends MusicBeatState
{
	...
	//all of the variables that can be created with Lua. the "variables" map is defined in MusicBeatState itself
	#if LUA_ALLOWED
	...
	//The FlxSound for both the instrumental and the vocals. opponentVocals can be null if there are none found in the song
	...
	//TODO: redirect healthBarBG and timeBarBG to these sprite's bg
	...
	//T-Bar Engine stuff
	public var countdownAnimation:Bool = true;
	public var timeBarColors:Bool = true;

	public var _traceLoadedScripts:Bool = false; //If you wanna see what scripts are loaded for the current song
	...
	override public function create()
	{
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'scripts/'))
			...
			{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith(Paths.LUA_EXT))
					luaArray.push(new FunkinLua(folder + file));
				#end

				#if HSCRIPT_ALLOWED
				if(file.toLowerCase().endsWith(Paths.HSCRIPT_EXT))
					hscriptArray.push(new HScript(folder + file, this));
				#end
			}
		#end
		...
		// STAGE SCRIPTS
		#if LUA_ALLOWED
		startLuasNamed('stages/' + curStage + Paths.LUA_EXT);
		#end

		#if HSCRIPT_ALLOWED
		startHScriptsNamed('stages/' + curStage + Paths.HSCRIPT_EXT);
		#end
		...
		for (notetype in noteTypes)
			startLuasNamed('custom_notetypes/' + notetype + Paths.LUA_EXT);
		for (event in eventsPushed)
			startLuasNamed('custom_events/' + event + Paths.LUA_EXT);
		#end

		#if HSCRIPT_ALLOWED
		for (notetype in noteTypes)
			startHScriptsNamed('custom_notetypes/' + notetype + Paths.HSCRIPT_EXT);
		for (event in eventsPushed)
			startHScriptsNamed('custom_events/' + event + Paths.HSCRIPT_EXT);
		#end
		noteTypes = null;
		eventsPushed = null;
		...
		if(_traceLoadedScripts && luaArray.length > 0) {
			var luaNameArray = [for(script in luaArray) script.scriptName];
			trace("Lua scripts loaded: " + luaNameArray.join(", "));
		}
		#end
		#if(MODS_ALLOWED && HSCRIPT_ALLOWED)
		if(_traceLoadedScripts && hscriptArray.length > 0) {
			var hscriptNameArray = [for(script in hscriptArray) script.scriptName];
			trace("Haxe scripts loaded: " + hscriptNameArray.join(", "));
		}
		#end
	...
	/*
	 * Loads a song in a new PlayState instance.
	 *
	 * @param	songName		The song name.
	 * @param	difficultyName	The song's difficulty.
	 * @param	storyModeSong	Is the song a story mode song?
	 */
	public static function loadSong(songName:String, ?difficultyName:String = "Normal", ?storyModeSong:Bool = false) {
		var songLowercase:String = Paths.formatToSongPath(songName);

		if(!backend.Difficulty.list.contains(difficultyName)) backend.Difficulty.list.push(difficultyName);
		var difficultyNum = backend.Difficulty.list.indexOf(difficultyName);
		if (difficultyNum == -1) difficultyNum = PlayState.storyDifficulty;

		var poop:String = Highscore.formatSong(songLowercase, difficultyNum);

		StageData.loadDirectory(PlayState.SONG);
		PlayState.SONG = Song.loadFromJson(poop, songName);
		PlayState.isStoryMode = storyModeSong;
	...
	function startCharacterScripts(name:String)
	{
		...
		var luaFile:String = 'characters/$name' + Paths.LUA_EXT;
		#if MODS_ALLOWED
		...
		#if HSCRIPT_ALLOWED
		var doPush:Bool = false;
		var scriptFile:String = 'characters/' + name + Paths.HSCRIPT_EXT;
		#if MODS_ALLOWED
		var replacePath:String = Paths.modFolders(scriptFile);

		if(#if sys FileSystem.exists(replacePath) #else Paths.fileExists(replacePath, TEXT) #end) {
			scriptFile = replacePath;
			doPush = true;
		} else {
			scriptFile = Paths.getSharedPath(scriptFile);
			if(#if sys FileSystem.exists(scriptFile) #else Paths.fileExists(scriptFile, TEXT) #end) {
				doPush = true;
			}
		}
		#else
		...
		if(doPush) {
			...
			if(doPush) hscriptArray.push(new HScript(scriptFile, this));
		}
		#end
	}
	...
	public function startVideo(name:String, ?skippable:Bool = true, ?loop:Bool = false)
	{
		...
		// Allows for using video links for startVideo
		// TODO: Allow for using Bytes
		...
		var videoCutscene:FunkinVideo = new FunkinVideo(skippable);
	...
	inline private function createCountdownSprite(image:String, antialias:Bool):FlxSprite
	{
		...
		if(countdownAnimation && !isPixelStage) {
			spr.scale.set(1.2, 1.2);
			FlxTween.tween(spr.scale, {x: 1, y: 1}, Conductor.crochet / 1000, {ease: FlxEase.cubeInOut});
		}
	...
	public dynamic function updateTimeTxt() {
	...
	function eventEarlyTrigger(event:EventNote):Float {
		// Weird issue, sorry.
		//(Edit: Might have something to do with hashlink arrays and casting)
		#if !hl
	...
	function resyncVocals(?vocalsAllowed:Bool = true):Void
	{
	...
	//Setting this to dynamic so people can set it to go to their own custom pause scripts
	dynamic function openPauseMenu()
	{
	...
	/*
	 * Custom Hardcoded events go here!
	 */
		...
		switch(eventName) {
			...
			case 'Set Property':
				try {
					...
					#if hl
					if(e == null || !Reflect.hasField(e, "message")) {
						trace(e);
						return;
					}
					#end
			...
			case 'Call Script':
				...
				for(arg in value2.split(',')) {
					args.push(switch(arg) {
						case "true" | "false": //Is a boolean
							(arg == "true");
						default: //Is a number or a string
							var _numbers = new EReg("^(?:[+-]?(?:(?:[1-9]\\d*|0)(?:\\.\\d*)?|\\.\\d+)(?:[Ee][+-]?\\d+)?)$", "");
							if(_numbers.match(arg)) Std.parseFloat(arg);
							else arg;
					});
				}
			...
			case 'Play Sound':
				FlxG.sound.play(Paths.sound(value1), flValue2 == null ? 1 : flValue2);
		}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!PlayState.isPixelStage)
		{
			...
			rating.angularAcceleration = FlxG.random.int(-100, 100) * playbackRate;
			comboSpr.setGraphicSize(Std.int(comboSpr.width * 0.7));
			comboSpr.angularAcceleration = FlxG.random.int(-50, 50) * playbackRate;
		}
		else
		{
	...
	/*
		This allows for resyncing the song and Conductor song position even if a song has no voices.
		TODO: Optimize this...
	*/
	public var canResyncVocals:Bool = true;
	public inline function checkSongSync(?hasVoices:Bool = true) {
		if(canResyncVocals && hasVoices && FlxG.sound.music.time >= -ClientPrefs.data.noteOffset) {
		...
		} else if(canResyncVocals && !hasVoices && FlxG.sound.music.time >= -ClientPrefs.data.noteOffset) {
			var timeSub:Float = Conductor.songPosition - Conductor.offset;
			var syncTime:Float = 20 * playbackRate;
			if(Math.abs(FlxG.sound.music.time - timeSub) > syncTime)
			{
	...
	{
		checkSongSync(SONG.needsVoices);
	...
	public function startHScriptsNamed(scriptFile:String)
	{
		...
		if(#if sys FileSystem.exists(scriptToLoad) #else Paths.fileExists(scriptToLoad, TEXT) #end)
		{
			for(sc in hscriptArray) {
				if(sc.scriptName == scriptToLoad) return false;
			}

			hscriptArray.push(new HScript(scriptToLoad, this));
			return true;
		}
		return false;
	}
