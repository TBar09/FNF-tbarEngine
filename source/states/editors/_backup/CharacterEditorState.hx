...
import states.editors.backend.CharacterUtil;
...
class CharacterEditorState extends EditorState
{
	...
	var isAnimateSprite(get, never):Bool;
	inline function get_isAnimateSprite():Bool {
		return (character != null && character.isAnimateAtlas);
	}
	...
	function addFileUI() {
		...
		var saveCharacterButton:FlxButton = new FlxButton(25, 15, "Save", function() {
			saveCharacter();
		});
		saveCharacterButton.color = FlxColor.GREEN;
		saveCharacterButton.label.color = FlxColor.WHITE;

		/* Saving in other formats */
		var saveCNECharacterButton:FlxButton = new FlxButton(saveCharacterButton.x, saveCharacterButton.y + 30, "Save CNE", function() {
			saveCharacter(CODENAME);
		});
		
		var saveVSliceCharacterButton:FlxButton = new FlxButton(saveCharacterButton.x, saveCNECharacterButton.y + 30, "Save VSlice", function() {
			saveCharacter(VSLICE);
		});

		/* Loading in other formats */
		var loadCNECharacterButton:FlxButton = new FlxButton(saveCNECharacterButton.x + saveCNECharacterButton.width + 20, saveCNECharacterButton.y, "Load CNE", function() {
			var _fileRef:FileBrowse = new FileBrowse();
			_fileRef.onComplete = function(fileData, fileName) {
				var charName = new haxe.io.Path(fileName).file;

				loadCharacter(CharacterUtil.convertFromCodename(fileData, charName));
			};

			_fileRef.start([{fileTypeName: "XML File", fileType: "*.xml"}], "Choose a Codename Character XML", "char.xml");
			_fileRef = null;
		});
		loadCNECharacterButton.color = FlxColor.YELLOW;
		loadCNECharacterButton.label.color = FlxColor.BLACK;
		
		var loadVSliceCharacterButton:FlxButton = new FlxButton(saveVSliceCharacterButton.x + saveVSliceCharacterButton.width + 20, saveVSliceCharacterButton.y, "Load VSlice", function() {
			var _fileRef:FileBrowse = new FileBrowse();
			_fileRef.onComplete = function(fileData, fileName) {
				var charName = new haxe.io.Path(fileName).file;

				loadCharacter(CharacterUtil.convertFromVSlice(fileData));
			};

			_fileRef.start([{fileTypeName: "JSON File", fileType: "*.json"}], "Choose a VSlice Character JSON", "char.json");
			_fileRef = null;
		});
		loadVSliceCharacterButton.color = FlxColor.YELLOW;
		loadVSliceCharacterButton.label.color = FlxColor.BLACK;

		var engineIcon:FlxSprite = new FlxSprite(loadCNECharacterButton.x + loadCNECharacterButton.width + 10, loadCNECharacterButton.y);
		engineIcon.loadGraphic(Paths.image('editors/chart-icons'), true, 20, 20);
		engineIcon.animation.add("idle", [1], 1, true);
		engineIcon.animation.play("idle");
		tab_group.add(engineIcon);

		var engineIcon:FlxSprite = new FlxSprite(loadVSliceCharacterButton.x + loadVSliceCharacterButton.width + 10, loadVSliceCharacterButton.y);
		engineIcon.loadGraphic(Paths.image('editors/chart-icons'), true, 20, 20);
		engineIcon.animation.add("idle", [0], 1, true);
		engineIcon.animation.play("idle");
		tab_group.add(engineIcon);

		tab_group.add(saveCharacterButton);
		tab_group.add(saveCNECharacterButton);
		tab_group.add(saveVSliceCharacterButton);
		tab_group.add(loadCNECharacterButton);
		tab_group.add(loadVSliceCharacterButton);
	...
	function addGhostUI()
	{
		...
		var ghostAlphaSlider:FlxUISlider = new FlxUISlider(this, 'ghostAlpha', 10, makeGhostButton.y + 25, 0, 1, 210, #if hl 15 #else null #end, 5, FlxColor.WHITE, FlxColor.BLACK);
	...
	function addSettingsUI()
	{
		...
		charDropDown = new FlxUIDropDownMenu(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray([''], true), function(index:String)
		{
			...
			var characterPath:String = 'characters/$intended';
			if(FileUtil.exists(Paths.getPath(characterPath + ".json", TEXT, null, true))) //T-Bar Engine & VSlice
			{
				_char = intended;
				check_player.checked = character.isPlayer;
				addCharacter();
			...
			else if(FileUtil.exists(Paths.getPath(characterPath + ".xml", TEXT, null, true))) //Codename Engine
			{
				_char = intended;
				check_player.checked = character.isPlayer;
				addCharacter();
	...
	function addCharacterUI()
	{
		...
		healthColorStepperR = new FlxUINumericStepper(singDurationStepper.x, noAntialiasingCheckBox.y + 40, 20, character.healthColorArray[0], 0, 255, 0);
		healthColorStepperG = new FlxUINumericStepper(singDurationStepper.x + 65, noAntialiasingCheckBox.y + 40, 20, character.healthColorArray[1], 0, 255, 0);
		healthColorStepperB = new FlxUINumericStepper(singDurationStepper.x + 130, noAntialiasingCheckBox.y + 40, 20, character.healthColorArray[2], 0, 255, 0);
	...
	function reloadCharacterDropDown() {
		...
		for (folder in foldersToCheck)
			for (file in FileSystem.readDirectory(folder))
				if(file.toLowerCase().endsWith('.json')) //T-Bar Engine & VSlice
				{
				...
				/*
				if(file.toLowerCase().endsWith('.xml')) //Codename Engine
				{
	...
	//Save/Load Stuff
	public inline function loadCharacter(charData:CharacterFile) {
		character.loadCharacterFile(charData);
		character.alpha = 1;
		reloadAnimList();
		reloadCharacterOptions();
		updateCharacterPositions();
	...
	public function saveCharacter(?charType:Null<CharacterType>) {
		var json:Dynamic = {
		...
		switch(charType) {
			case CODENAME:
				var data:String = CharacterUtil.convertToCodename(json);
				if(data != null && data.length > 0) {
					var _file:FileSave = new FileSave();
					_file.start(data, '${_char}.xml');
				}

			case VSLICE:
				var data:String = CharacterUtil.convertToVSlice(json, (isAnimateSprite ? "animateatlas" : "sparrow"));
				if(data != null && data.length > 0) {
					var _file:FileSave = new FileSave();
					_file.start(data, '${_char}.json');
				}
