...
class NotesSubState extends MusicBeatSubstate
{
	...
	var _noteDesignFile:FileReference;
	...
	var saveDesignButton:ClickableButton;
	var loadDesignButton:ClickableButton;
	...
	public function new() {
		...
		saveDesignButton = new ClickableButton(20, FlxG.height - 150, "Save Note Design", function() {
			saveNoteDesign();
		}, 15, "funkin.ttf");
		add(saveDesignButton);

		loadDesignButton = new ClickableButton(saveDesignButton.x, saveDesignButton.y + 40, "Load Note Design", function() {
			loadNoteDesign();
		}, 15, "funkin.ttf");
		add(loadDesignButton);
	...
	function saveNoteDesign():Bool {
		try {
			var noteObj:NoteDesign = {
				rgbColors: [],
				pixelNotes:	onPixel,
				generatedBy: ("T-Bar Engine v" + openfl.Lib.application.meta["version"])
			};

			for(i => item in dataArray) {
				noteObj.rgbColors.push([item[0], item[1], item[2]]);
			}

			//var note_design_string = #if tjson tjson.TJSON.encode(noteObj, 'fancy') #else haxe.Json.stringify(noteObj, "\t") #end;
			var note_design_string = haxe.Json.stringify(noteObj, "\t");

			if(note_design_string != null && note_design_string.length > 0) {
				_noteDesignFile = new FileReference();
				_noteDesignFile.addEventListener(#if desktop Event.SELECT #else Event.COMPLETE #end, onSaveComplete);
				_noteDesignFile.addEventListener(Event.CANCEL, onSaveCancel);
				_noteDesignFile.addEventListener(IOErrorEvent.IO_ERROR, onSaveError);
				_noteDesignFile.save(note_design_string.trim(), "my note design.json");
				return true;
			}
		} catch(e) {
			trace("Error saving Note Design: " + Std.string(e));
		}
		return false;
	}
	
	function loadNoteDesign():Bool {
		try {
			_noteDesignFile = new FileReference();
			_noteDesignFile.addEventListener(Event.SELECT, on_load);
			_noteDesignFile.browse([new FileFilter("JSON file", "*.json")]);
			return true;
		} catch(e) {
			trace("Error loading Note Design: " + Std.string(e));
		}
		return false;
	}

	function onLoadFile(_):Void {
		try {
			var designStr = _noteDesignFile.data.toString();
			var __noteObj__ = #if tjson tjson.TJSON.parse(designStr) #else haxe.Json.parse(designStr) #end;

			dataArray = __noteObj__.rgbColors;
			for(index => _rgbPallete in dataArray) {
				if(__noteObj__.pixelNotes) ClientPrefs.data.arrowRGBPixel[index] = _rgbPallete;
				else ClientPrefs.data.arrowRGB[index] = _rgbPallete;
			}
			...
			_noteDesignFile.removeEventListener(Event.SELECT, on_load);
			_noteDesignFile.removeEventListener(Event.COMPLETE, onLoadFile);
			_noteDesignFile = null;
		} catch(e) {
			trace("Error loading Note Design: " + Std.string(e));
		}
	}
	...
	{
		_noteDesignFile.addEventListener(Event.COMPLETE, onLoadFile);
		_noteDesignFile.load();
	}

	function onSaveComplete(_):Void
	{
		_noteDesignFile.removeEventListener(Event.COMPLETE, onSaveComplete);
		_noteDesignFile.removeEventListener(Event.CANCEL, onSaveCancel);
		_noteDesignFile.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_noteDesignFile = null;
		FlxG.log.notice("Successfully saved note design");
	}
	...
	/* Called if there is an error while saving */
	function onSaveError(_):Void
	{
		...
		_noteDesignFile.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_noteDesignFile = null;
		FlxG.log.error("Problem saving note design");
	}
}

typedef NoteDesign = {
	var rgbColors:Array<Array<Int>>;
	var pixelNotes:Bool;
	@:optional var generatedBy:String;
}

class ClickableButton extends FlxSpriteGroup {
	public var bg:FlxSprite;
	public var txt:FlxText;
	public var borderSize:Float = 5;
	public var text(get, set):String; //Quick access
	public var onClick:()->Void;

	override public function new(x:Float = 0, y:Float = 0, text:String, ?onClick:()->Void, ?size:Int = 12, ?fontPath:String = "pixel.otf") {
		super(x, y);
		...
		txt = new FlxText(this.x + borderSize, this.y + borderSize, 0, text, size);
		txt.setFormat(Paths.font(fontPath), size, FlxColor.WHITE, "center", FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		this.text = text; //Just so the button scaling stuff gets called
	...
	override function update(elapsed:Float) {
		...
		if(FlxG.mouse.overlaps(this.bg)) {
			bg.alpha = 0.4;
			txt.alpha = 0.7;
			if(FlxG.mouse.justPressed && this.onClick != null) {
				this.onClick();
			}
		} else {
			bg.alpha = 0.6;
	...
	override function draw() {
        super.draw();
        txt.setPosition(this.x + borderSize, this.y + borderSize);
        txt.draw();
    }

	function get_text():String { return txt.text; }
	function set_text(val:String):String {
		txt.text = val;
		bg.scale.set(txt.width + (borderSize * 2), txt.height + (borderSize * 2));
		bg.updateHitbox();
