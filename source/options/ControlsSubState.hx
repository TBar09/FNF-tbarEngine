...
class ControlsSubState extends MusicBeatSubstate
{
	...
	function addKeyText(text:Alphabet, option:Array<Dynamic>, id:Int)
	{
		for (n in 0...2)
		{
			...
			{
				#if hl
				var savKey:Dynamic = ClientPrefs.keyBinds.get(option[2]);
				#else
			...
			{
				#if hl
				var savKey:Dynamic = ClientPrefs.gamepadBinds.get(option[2]);
				#else
			...
			else
			{
				...
				if(changed)
				{
					...
					for (n in 0...2)
					{
						...
						{
							#if hl
							var savKey:Dynamic = ClientPrefs.keyBinds.get(option);
							#else
						...
						{
							#if hl
							var savKey:Dynamic = ClientPrefs.gamepadBinds.get(option);
							#else
