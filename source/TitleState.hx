...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		#if MODS_ALLOWED
		Discord.DiscordClient.loadModRPC();
		#end
		...
		if(!initialized)
		{
			...
			if(ClientPrefs.windowMode == 'DARK') CppAPI.setWindowColorMode(true, CppAPI.hasWindowsVersion("10"));
			#end
	...
	function startIntro()
	{
		...
		psychSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image(#if TBAR_WATERMARKS 'engine/psychEngine' #else 'newgrounds_logo' #end));
		add(psychSpr);
		...
		tbarSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('engine/tbarEngine'));
		add(tbarSpr);
		tbarSpr.visible = false;
		tbarSpr.setGraphicSize(Std.int(tbarSpr.width * 0.8));
		tbarSpr.updateHitbox();
		tbarSpr.screenCenter(X);
		tbarSpr.antialiasing = ClientPrefs.globalAntialiasing;
	...
	{
		var fullText:String = backend.OnlineUtil.onlineData.get("introText")[0];
	...
	var COOLBEATS:FlxTween;
	override function beatHit()
	{
		super.beatHit();
		
		if(skippedIntro && titleJSON.cameraBop)
		{
			FlxG.camera.zoom += 0.05;
			
			COOLBEATS = FlxTween.tween(FlxG.camera, {zoom: 1}, 0.45, {ease: FlxEase.quadOut});
		}
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if TBAR_WATERMARKS
					createCoolText(['TBar Engine by'], 15);
					#else
				...
				case 4:
					#if TBAR_WATERMARKS
					addMoreText('T-Bar', 15);
					tbarSpr.visible = true;
					#else
				...
				case 8:
					#if TBAR_WATERMARKS
					createCoolText(['Psych Engine'], 90);
					#else
		...
		setOnMenuScript("curBeat", sickBeats);
	}
	...
	{
		#if SOFTCODED_STATES
		var ret = callOnMenuScript("onSkipIntro", [(!skippedIntro)]);
		#end
		if (!skippedIntro #if SOFTCODED_STATES && ret != FunkinLua.Function_Stop #end)
		{
