...
class DeprecatedFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		// Old T-Bar Engine deprecated functions that are replaced by better methods
		Lua_helper.add_callback(lua, "makeLuaBackdrop", function(tag:String, ?image:String = null, ?x:Float = 0, ?y:Float = 0, axis:String = 'xy', xSpacing:Int = 0, ySpacing:Int = 0) {
			FunkinLua.luaTrace("makeLuaBackdrop is deprecated! Use createInstance instead", false, true);

			tag = tag.replace('.', '');
			LuaUtils.resetSpriteTag(tag);

			var leSprite:ModchartBackdrop = new ModchartBackdrop(null, LuaUtils.axisFromString(axis), xSpacing, ySpacing);
		...
		Lua_helper.add_callback(lua, "addLuaBackdrop", function(tag:String, front:Bool = false) {
			FunkinLua.luaTrace("addLuaBackdrop is deprecated! Use addInstance instead", false, true);

			var mySprite:FlxBackdrop = null;
			if(PlayState.instance.modchartBackdrops.exists(tag)) mySprite = PlayState.instance.modchartBackdrops.get(tag);
		...
		Lua_helper.add_callback(lua, "luaBackdropExists", function(tag:String) {
			FunkinLua.luaTrace("luaBackdropExists is deprecated", false, true);
		...
		Lua_helper.add_callback(lua, "removeLuaBackdrop", function(tag:String, destroy:Bool = true) {
			FunkinLua.luaTrace("removeLuaBackdrop is deprecated! Use removeInstance instead", false, true);
