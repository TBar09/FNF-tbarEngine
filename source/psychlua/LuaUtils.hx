...
class LuaUtils
{
	...
	public static function getModSetting(saveTag:String, ?modName:String = null)
	{
		...
		if(FileSystem.exists(path))
		{
			if(settings == null || !settings.exists(saveTag))
			{
				...
				catch(e:Dynamic)
				{
					...
					#if windows
					FlxG.stage.window.alert(errorMsg, errorTitle);
					#end
		...
		else
		{
			...
			LuaUtils.hxTrace('getModSetting: $path could not be found!', FlxColor.RED);
			#else
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		LuaUtils.hxTrace('getModSetting: "$saveTag" could not be found inside $modName\'s settings!', FlxColor.RED);
		#else
	...
	inline public static function getBuildLanguage():String {
		#if cpp
		return 'c++';
		#elseif cppia
		return 'cppia';
	...
	public static function getTextBorderFromString(?border:String = "") {
		switch(border.toLowerCase().trim()) {
			case "shadow": return FlxTextBorderStyle.SHADOW;
			case "outline"|"border": return FlxTextBorderStyle.OUTLINE;
			case "outline_fast"|"outlinefast": return FlxTextBorderStyle.OUTLINE_FAST;
		}
		return FlxTextBorderStyle.NONE;
	}

	public static function axisFromString(axis:String):FlxAxes {
		switch(axis.toLowerCase().trim()) {
			case "x": return FlxAxes.X;
			case "y": return FlxAxes.Y;
			case "none": return FlxAxes.NONE;
		}
		return FlxAxes.XY;
	}
	...
	public static function hxTrace(text:Dynamic, color:FlxColor) { //temporary
		#if LUA_ALLOWED
		FunkinLua.luaTrace(Std.string(text), true, false, color);
	...
	/* //Nvm this is stupid
	public static function getHScriptScriptObject() {
		if(FlxG.state.subState == null) { //there is no substate so use the state
			if(Reflect.hasField(FlxG.state, "instance")) 
				return Reflect.field(FlxG.state, "instance");
			else
				return FlxG.state;
		} else { //There is a substate
			if(Reflect.hasField(FlxG.state.subState, "instance")) 
				return Reflect.field(FlxG.state.subState, "instance");
			else
	...
	*/
	...
	public static dynamic function getHScriptPreprocessors() {
		var preprocessors:Map<String, Dynamic> = backend.system.macros.CompilerMacro.defines;
		...
		preprocessors.set("INITIAL_STATE", CoolUtil.getStateName(FlxG.state));

		#if HSCRIPT_ALLOWED //for compatibility with older versions
		preprocessors.set("HXSCRIPT_ALLOWED", "1");
		#end

		#if(cpp && windows) //hxWindowColorMode functionality
		preprocessors.set("hxWindowColorMode", "1");
		#end

		#if LUA_ALLOWED
		preprocessors.set("luajit", StringTools.replace(Lua.versionJIT(), "LuaJIT ", "").trim());
		preprocessors.set("lua", StringTools.replace(Lua.version(), "Lua ", "").trim());
		#end
	...
	/**
	 * Gets the macro class created by hscript-improved for abstracts
	 */
