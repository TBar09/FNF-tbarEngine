...
class FunkinLua implements ScriptInterface {
	...
	public static var customVariables:Map<String, Dynamic> = new Map<String, Dynamic>();
	...
	public function new(scriptName:String) {
		...
		set('version', "tbarEngine " + Application.current.meta.get('version').trim());
		set('versionTBarEngine', Application.current.meta.get('version').trim());

		// build target (windows, mac, linux, etc.)
		set('buildTarget', LuaUtils.getBuildTarget());
		set('buildLanguage', LuaUtils.getBuildLanguage());
		...
		for(name => val in customVariables) {
			if(LuaUtils.isLuaSupported(val)) set(name, val);
		}
		...
		if(FlxG.state is PlayState) {
			setPlayStateCallbacks(PlayState.instance);
			ScriptFunctions.implement(this, PlayState.instance);

			#if flxanimate FlxAnimateFunctions.implement(this); #end
			FunkinVideo.addLuaCallbacks(lua, PlayState.instance);
			...
			ExtraFunctions.addTBarEngineCallbacks(lua);
			ExtraFunctions.addGhostutilWindowCallbacks(lua);
		}
	...
	inline function setPlayStateCallbacks(game:PlayState) {
		if(game == null) return;

		/* PlayState exclusive functions */
		...
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?type:String) {
			oldTweenFunction(tag, vars, {x: value}, duration, ease, 'doTweenX', type);
		});
		Lua_helper.add_callback(lua, "doTweenY", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?type:String) {
			oldTweenFunction(tag, vars, {y: value}, duration, ease, 'doTweenY', type);
		});
		Lua_helper.add_callback(lua, "doTweenAngle", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?type:String) {
			oldTweenFunction(tag, vars, {angle: value}, duration, ease, 'doTweenAngle', type);
		});
		Lua_helper.add_callback(lua, "doTweenAlpha", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?type:String) {
			oldTweenFunction(tag, vars, {alpha: value}, duration, ease, 'doTweenAlpha', type);
		});
		Lua_helper.add_callback(lua, "doTweenZoom", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, ?type:String) {
			oldTweenFunction(tag, vars, {zoom: value}, duration, ease, 'doTweenZoom', type);
		});
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String, ?type:String) {
			var penisExam:Dynamic = LuaUtils.tweenPrepare(tag, vars);
			if(penisExam != null) {
				...
				game.modchartTweens.set(tag, FlxTween.color(penisExam, duration, curColor, CoolUtil.colorFromString(targetColor), {
					...
					type: LuaUtils.getTweenTypeByString(type), 
					onComplete: function(twn:FlxTween) {
		...
		Lua_helper.add_callback(lua, "noteTweenX", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, ?type:String) {
		...
		Lua_helper.add_callback(lua, "noteTweenY", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, ?type:String) {
			LuaUtils.cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = game.strumLineNotes.members[note % game.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "noteTweenAngle", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, ?type:String) {
			LuaUtils.cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = game.strumLineNotes.members[note % game.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "noteTweenDirection", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, ?type:String) {
			LuaUtils.cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = game.strumLineNotes.members[note % game.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "noteTweenAlpha", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, ?type:String) {
			LuaUtils.cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = game.strumLineNotes.members[note % game.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "screenCenter", function(obj:String, pos:String = 'xy') {
			...
			if(spr != null) {
				spr.screenCenter(LuaUtils.axisFromString(pos.trim()));
				return;
			}
	...
	//Taken from Nightmarevision I have little idea how this lua stuff works (credits to DuskieWhy)
	...
	function oldTweenFunction(tag:String, vars:String, tweenValue:Any, duration:Float, ease:String, funcName:String, type:String)
	{
	...
	public function findScript(scriptFile:String, ext:String = Paths.LUA_EXT)
	{
		//if(ext == null || ext.length <= 0) ext = Paths.LUA_EXT;
	...
	public function getErrorMessage(status:Int):String {
		...
		if (v == null || v == "") {
			switch(status) {
				...
				case Lua.LUA_ERRSYNTAX: return "Syntax Error";
			}
	...
	#if (MODS_ALLOWED && SHADERS_ALLOWED && sys)
	...
	#if HSCRIPT_ALLOWED
	public function initHaxeModule(?funkin:FunkinLua) {
		if(funkin != null) {
			//trace('initializing haxe interp for: $scriptName');
			this.hscript = new HaxeCode(funkin);
		}
	}
	#end
}
