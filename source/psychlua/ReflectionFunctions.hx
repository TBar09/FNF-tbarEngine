...
class ReflectionFunctions
{
	...
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "setProperty", function(variable:String, value:Dynamic, allowMaps:Bool = false, ?allowInstances:Bool = false) {
			var split:Array<String> = variable.split('.');
			if(split.length > 1) {
				LuaUtils.setVarInArray(LuaUtils.getPropertyLoop(split, true, true, allowMaps), split[split.length-1], (allowInstances ? parseInstances(value) : value), allowMaps);
				return true;
			}
		...
		Lua_helper.add_callback(lua, "removeInstance", function(objectName:String, destroy:Bool = true) {
			...
			{
				var obj:Dynamic = PlayState.instance.variables.get(objectName);
				if(destroy && Reflect.field(obj, "kill") != null) obj.kill();

				LuaUtils.getTargetInstance().remove(obj);
				if(destroy) {
					if(Reflect.field(obj, "destroy") != null) obj.destroy();
					PlayState.instance.variables.remove(obj);
				}
			}
			else FunkinLua.luaTrace('removeInstance: Can\'t remove what doesn\'t exist~ ($objectName)', false, false, FlxColor.RED);
		});
		...
		Lua_helper.add_callback(lua, "hasField", function(variable:String, field:String, ?allowMaps:Bool = false):Bool {
			var split:Array<String> = (variable + "." + field).split('.');
			var retVal = null;
			if(split.length > 1)
				retVal = LuaUtils.getVarInArray(LuaUtils.getPropertyLoop(split, true, true, allowMaps), field, allowMaps);
			else 
				retVal = LuaUtils.getVarInArray(LuaUtils.getTargetInstance(), (variable + "." + field), allowMaps);
	...
	public static function resolve(className:String, ?allowAbstracts:Bool = true):Null<Dynamic> {
		var clsObj = Type.resolveClass(className);
		return ((allowAbstracts && clsObj == null) ? Type.resolveClass('${className}_HSC') : clsObj);
	}
}