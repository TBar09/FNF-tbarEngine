...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		FlxTween.tween(grid, {alpha: 0.24}, 0.5, {ease: FlxEase.quartInOut});
		...
		// missingTextBG = bg.clone();
		if(menuItems.contains("Change Difficulty")) {
	...
	override function update(elapsed:Float)
	{
		...
		switch(daSelected) {
			case 'Skip Time':
				if (controls.UI_LEFT_P || controls.UI_RIGHT_P)
				{
					...
					curTime += (controls.UI_LEFT_P ? -1000 : 1000);
					holdTime = 0;
				}
		...
		if (controls.ACCEPT && (cantUnpause <= 0 || !controls.controllerMode)) {
			if (menuItems == difficultyChoices) { //For "Change Difficulty"
				try {
					...
					if(missingText != null && missingTextBG != null) {
	...
	public static function restartSong(noTransition:Bool = false) {
		PlayState.instance.paused = true; // For lua
		FlxG.sound.music.volume = 0;
		PlayState.instance.vocals.volume = 0;
	...
	function changeSelection(delta:Int = 0):Void {
