...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "NO MODS INSTALLED\nPRESS [ESCAPE] AND INSTALL A MOD!", 48);
		...
		buttonSettings = new FlxButton(startX, 0, "MOD SETTINGS", function()
		{
			var curMod:ModMetadata = mods[curSelected];
		...
		buttonSettings.color = 0xFF404040;
		buttonSettings.setGraphicSize(200, 50);
		buttonSettings.updateHitbox();
		add(buttonSettings);
		buttonsArray.push(buttonSettings);
		visibleWhenHasMods.push(buttonSettings);
		buttonSettings.label.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER);
		buttonSettings.label.fieldWidth = 190;
		setAllLabelsOffset(buttonSettings, 5, 10);
		startX -= 70;
	...
	function updateButtonToggle()
	{
		...
		buttonToggle.color = modsList[curSelected][1] ? 0xFF008000 : 0xFFFF0000;
	}
	...
	function changeSelection(change:Int = 0)
	{
		...
		for (mod in mods)
		{
			...
			if(i == curSelected)
			{
				...
				descriptionTxt.text = mod.description + (mod.restart ? " (This Mod will restart the game!)" : "");//finna make it to where if nothing changed then it won't reset
