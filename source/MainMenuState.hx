...
class MainMenuState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		lerpVal = CoolUtil.boundTo(elapsed * 7.5, 0, 1);
		...
		if (!selectedSomethin)
		{
			...
			#if SOFTCODED_STATES
			else if(FlxG.keys.justPressed.TAB) { //Failsafe in case you remove the mods menu from a custom menu
				selectedSomethin = true;
				FlxG.sound.play(Paths.sound('cancelMenu'));
	...
	{	
		if(callOnMenuScript("onChangeItemPre", [huh]) != FunkinLua.Function_Stop) {
		...
		callOnMenuScript("onChangeItem", [huh]);
	}
}