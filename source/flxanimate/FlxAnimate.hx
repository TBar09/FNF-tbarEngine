...
class FlxAnimate extends FlxSprite
{
	...
	public function loadAtlasEx(img:FlxGraphicAsset, pathOrStr:String = null, myJson:Dynamic = null)
	{
		...
		if(myJson is String)
		{
			...
			if(trimmed == '.json') myJson = _getFileContent(myJson); //is a path
		...
		{
			myData = _getFileContent(pathOrStr);
			isXml = false;
		}
	...
	function _getFileContent(filepath:String) {
		#if sys
		return sys.io.File.getContent(filepath);
		#else
		return openfl.Assets.getText(filepath);
		#end
	}
}
