...
class PlayState extends MusicBeatState
{
	...
	public static var playstateActive:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'school': //Week 6 - Senpai, Roses
				...
				var bgStreet:BGSprite = new BGSprite('weeb/weebStreet', repositionShit, 0, 1, 1);
				add(bgStreet);
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				var posY = 200;
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('weeb/animatedEvilSchool', posX, posY, 1, 1, ['background 2'], true);
					bg.scale.set(6, 6);
					...
					var bg:BGSprite = new BGSprite('weeb/animatedEvilSchool_low', posX, posY, 1, 1);
					bg.scale.set(6, 6);
					bg.antialiasing = false;
					add(bg);
				}
		...
		RPCDetails = #if desktop SONG.song + ' (' + detailsText + ')' #else SONG.song #end;
		RPCState = null;
		...
		botplayTxt.angle = 5;
		FlxTween.tween(botplayTxt, {angle: -5}, Conductor.crochet / 1000, {ease: FlxEase.cubeInOut, type: PINGPONG});
		botplayTxt.visible = cpuControlled;
		add(botplayTxt);
		if(ClientPrefs.downScroll) botplayTxt.y = timeBarBG.y - 78;
		
		//Highscore.getScore(SONG.song, storyDifficulty)
		...
		DiscordClient.changePresence(RPCDetails, RPCState, iconP2.getCharacter());
		#end
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					...
					case 1:
						...
						{
							countdownReady.scale.x = 1.1;
							countdownReady.scale.y = 1.1;
							FlxTween.tween(countdownReady.scale, {x: 1, y: 1}, Conductor.crochet / 1000, {ease: FlxEase.cubeInOut});
						}
					...
					case 2:
						...
						if(countdownAnimation && !isPixelStage)
						{
							countdownSet.scale.x = 1.15;
							countdownSet.scale.y = 1.15;
							FlxTween.tween(countdownSet.scale, {x: 1, y: 1}, Conductor.crochet / 1000, {ease: FlxEase.cubeInOut});
						}
					...
					case 3:
						...
						if(countdownAnimation && !isPixelStage)
						{
							...
							FlxTween.tween(countdownGo.scale, {x: 1, y: 1}, Conductor.crochet / 1000, {ease: FlxEase.cubeInOut});
							FlxTween.tween(countdownGo, {angle: 360}, (Conductor.crochet / 1000), {
								ease: FlxEase.cubeInOut
							});
						}
						FlxTween.tween(countdownGo, {alpha: 0}, 0.5 + (Conductor.crochet / 1000), {
							ease: FlxEase.cubeInOut,
							onComplete: function(twn:FlxTween)
							{
								remove(countdownGo);
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				DiscordClient.changePresence(RPCDetails, RPCState, iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(RPCDetails, RPCState, iconP2.getCharacter());
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(RPCDetails, "Game Paused", iconP2.getCharacter());
		}
		#end
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence(RPCDetails, "Game Over!", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
