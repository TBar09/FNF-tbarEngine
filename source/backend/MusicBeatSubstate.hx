...
class MusicBeatSubstate extends FlxSubState
{
	...
	override function create() {
		#if (SOFTCODED_STATES && HSCRIPT_ALLOWED)
		if(scriptsAllowed) runSubstateFiles((useCustomStateName ? className : CoolUtil.getStateName(this)));
		#end
	...
	override function close() {
		if (callOnMenuScript("onClose", []) != LuaUtils.Function_Stop) {
			super.close();
			callOnMenuScript("onClosePost", []);
		}
	}
	...
	public function runSubstateFiles(substate:String) {
		if(!scriptsAllowed) return;
		//trace(substate);

		#if sys
		var filesPushed = [];
		for (folder in Paths.getStateScripts(substate)) {
