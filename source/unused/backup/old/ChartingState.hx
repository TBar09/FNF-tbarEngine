...
import states.editors.charting.ChartConvertUtil;
import states.editors.charting.FileBrowser;
...
class ChartingState extends MusicBeatState
{
	 ...
	 * Though, you should add your hardcoded custom note types here too.
	 */
	...
	/* 
	 * Events here will be available for the chart editor to use. Includes name & description.
	 * If you added a hardcoded event in PlayState, add it here so it can be used!
	 */
	var eventStuff:Array<Dynamic> = [
		...
		['Call Script', "Value 1: The Function Name\nValue 2: The arguments, separated by commas [,].\nCan be a number, string, or a boolean value.\n\nCalls a function from all running scripts."],
	...
	**/
	...
	override function create()
	{
		...
		#if(CRASH_HANDLER == "tbar")
		crashSaveEnabled = true;
		#end
		super.create();
	}

	/* Adds characters to the chart editor characters drop-down. */
	inline public function addCharactersToList():Array<String> {
		#if MODS_ALLOWED
		...
		var _temp_characters:Array<String> = Mods.mergeAllTextsNamed('data/characterList.txt', Paths.getSharedPath());
		for (character in _temp_characters)
		{
				...
				for (file in FileSystem.readDirectory(directory)) {
					...
					if (!FileSystem.isDirectory(path) && file.endsWith('.json')) {
						...
						if(charToCheck.trim().length > 0 && !charToCheck.endsWith('-dead') && !tempArray.contains(charToCheck)) {
							...
							_temp_characters.push(charToCheck);
						}
					}
				}
			}
	...
	/* Adds the stages to the chart editor "stages" drop-down. */
	inline public function addStagesToList():Array<String> {
		#if MODS_ALLOWED
		...
		var tempArray:Array<String> = [];
		var _temp_stages:Array<String> = [];

		for (stage in stageFile) {
			if(stage.trim().length > 0) {
				_temp_stages.push(stage);
			}
				...
				for (file in FileSystem.readDirectory(directory)) {
					...
					if (!FileSystem.isDirectory(path) && file.endsWith('.json')) {
						...
						if(stageToCheck.trim().length > 0 && !tempArray.contains(stageToCheck)) {
							...
							_temp_stages.push(stageToCheck);
						}
					}
				}
			}
	...
	/* Adds the events to the chart editor "events" drop-down. */
	inline public function addEventsToList() {
		#if LUA_ALLOWED
	...
	function addFileUI():Void {
		...
		saveAsTxt = new FlxText(10, 20, 0, "Save As:");
		asVSliceButton = new FlxButton(saveAsTxt.x, saveAsTxt.y + 30, "VSlice Chart", function() {
			saveLevelAs(VSLICE);
		});
		asCNEButton = new FlxButton(asVSliceButton.x, asVSliceButton.y + 30, "CNE Chart", function() {
			saveLevelAs(CODENAME);
		});

		loadFromTxt = new FlxText(UI_box.width/2, 20, 0, "Load Chart From:");
		fromVSliceButton = new FlxButton(loadFromTxt.x, loadFromTxt.y + 30, "VSlice Chart", function() {
			loadLevelFrom(VSLICE, true);
		});
		fromCNEButton = new FlxButton(fromVSliceButton.x, fromVSliceButton.y + 30, "CNE Chart", function() {
	...
	public final _supportedFileTypes:Array<FileTypeFilter> = [
	...
	var __current_chart_data:String;
	var __current_chart_metadata:String;
	public function loadLevelFrom(type:ChartType, ?hasMetadata:Bool = false) {
		var _fileBrowse = new FileBrowse();
		_fileBrowse.onComplete = function(fileContents) {
			...
			if(hasMetadata) {
				...
				var _fileBrowse = new FileBrowse();
				_fileBrowse.onComplete = function(metadataContents) {
					__current_chart_metadata = metadataContents.trim();
					var engineChart = null;

					switch(type) {
						case VSLICE:
							engineChart = ChartConvertUtil.convertFromVSlice(__current_chart_data, __current_chart_metadata, Paths.formatToSongPath(Difficulty.getString()));
						case CODENAME:
							engineChart = ChartConvertUtil.convertFromCodename(__current_chart_data, __current_chart_metadata, Paths.formatToSongPath(Difficulty.getString()));
					}
				...
				_fileBrowse.start(_supportedFileTypes, "Select a Chart Metadata File");
				return;
			}

			//There are currently no supported formats here that have no metadata so we just don't load anything
		...
		_fileBrowse.start(_supportedFileTypes, (hasMetadata ? "Select a Chart Data File" : "Select a Chart File"));
	}

	function reloadWithChart(data:Dynamic, ?meta:Dynamic) {
		PlayState.SONG = data;
	...
	public function saveLevelAs(type:ChartType) {
		...
		if((data != null) && (data.length > 0)) {
			var _folderBrowse = new FolderBrowse();
			_folderBrowse.onComplete = function(path) {
				if(path == null) return;

				switch(type) {
					case VSLICE:
						ChartConvertUtil.convertToVSlice(Paths.formatToSongPath(_song.song), data.trim(), path, Paths.formatToSongPath(Difficulty.getString()));
					case CODENAME:
						ChartConvertUtil.convertToCodename(Paths.formatToSongPath(_song.song), data.trim(), path, Paths.formatToSongPath(Difficulty.getString()));
				}
			};
	...
	function addSongUI():Void
	{
		...
		reloadSong.label.color = FlxColor.BLACK;
		...
		reloadSongJson.label.color = FlxColor.BLACK;
		...
		var characters:Array<String> = addCharactersToList();
	...
	function addNoteUI():Void
	{
		...
		for (folder in foldersToCheck)
			for (file in FileSystem.readDirectory(folder))
			{
				...
				if(
					(#if LUA_ALLOWED (fileName.endsWith(Paths.LUA_EXT) && (wordLen = Paths.LUA_EXT.length) == Paths.LUA_EXT.length) || #end
					#if HSCRIPT_ALLOWED (fileName.endsWith(Paths.HSCRIPT_EXT) && (wordLen = Paths.HSCRIPT_EXT.length) == Paths.HSCRIPT_EXT.length) || #end
	...
	function addEventsUI():Void
	{
		...
		addEventsToList(); //Adds custom events to the event list

		descText = new FlxText(20, 200, 0, eventStuff[0][0]);
		//descText.fieldWidth = 280;
	...
	#if(CRASH_HANDLER == "tbar")
	var save_on_crash:FlxUICheckBox;
	#end
	function addChartingUI() {
		...
		#if(CRASH_HANDLER == "tbar")
		save_on_crash = new FlxUICheckBox(130, 190, null, null, "Autosave Chart on Crash", 100,
			function() {
				crashSaveEnabled = save_on_crash.checked;
			});
		save_on_crash.checked = crashSaveEnabled;
		tab_group_chart.add(save_on_crash);
		#end
	...
	function updateGrid():Void
	{
		...
		if(_song != null && _song.notes != null && _song.notes[curSec] != null
			&& _song.notes[curSec].changeBPM && _song.notes[curSec].bpm > 0) {
	...
	//Saves your chart to an external folder if your game crashes while your charting
	function crashSave(errorName:String, errorMsg:String) {
		if(_song.events != null && _song.events.length > 1) _song.events.sort(sortByTime);
		var json = {"song": _song};
