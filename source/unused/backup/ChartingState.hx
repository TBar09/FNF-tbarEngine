...
import states.editors.charting.ChartUtil;
...
enum ChartType {
	VSLICE;
	CODENAME;
	PSYCH; //Do not be fooled. This is for Psych 1.0
}
...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addFileUI():Void {
		...
		asPsychButton = new FlxButton(asCNEButton.x, asCNEButton.y + 30, "Psych 1.0", function() {
			saveLevelAs(PSYCH);
		});
		...
		_fileBrowse.onComplete = function(fileContents) {
			...
			switch(type) {
				case PSYCH:
					...
					engineChart = ChartUtil.convertFromPsych(__current_chart_data, Paths.formatToSongPath(Difficulty.getString()));
				default:
					engineChart = null;
			}
		...
		if(data != null && data.length > 0) {
			...
			_folderBrowse.onComplete = function(path) {
				...
				switch(type) {
					...
					case PSYCH:
						ChartUtil.convertToPsych(Paths.formatToSongPath(_song.song), data.trim(), path, Paths.formatToSongPath(Difficulty.getString()));
				}
			};
